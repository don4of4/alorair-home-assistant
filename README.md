...
The 0.3.1 update registers the **same 24 entities** in both profiles, including optional diagnostics disabled by default. Use the existing integration entry's **Reconfigure** action: entity names, IDs and user customizations remain attached to the same device. The Power switch is available in either profile and shares the dehumidifier's command handling. Since 0.4.0 the measured inlet/outlet sensors report in either profile, and the dehumidifier's current humidity is the inlet reading rather than its target.
