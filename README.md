...
The local profile for the **unreleased 0.3.0 build** has been partially commissioned and is **not ready for unattended production use**; v0.2.2 contains only the cloud profile. Home Assistant retained the dehumidifier entity identity and confirmed display changes, ON/OFF and targets 50%/55%. A valid continuous-mode request timed out and the reported target remained 55 before confirmed OFF. Earlier standalone continuous-mode restoration succeeded, so command reliability remains unresolved. Intake humidity, faults, purge and locator are unavailable locally; sustained offline operation, cold boot and persistent offline DNS remain unverified. [Local setup and evidence](docs/LOCAL_CONTROL.md)
