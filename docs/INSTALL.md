...
For a device already configured with Cloud, use the existing entry's **Reconfigure** action. Duplicate device identities are rejected. Reconfiguration preserves the entry/device identity and the dehumidifier's unique identity, and replaces cloud credentials with local settings. The 0.3.1 update registers the same 24 entities for both profiles, including optional diagnostics disabled by default. Fourteen currently work locally: dehumidifier including its current humidity, Power switch, Temperature display, Fresh device sample, Device sample time, Last command, and the measured Inlet/Outlet humidity, temperature and specific-humidity sensors added in 0.4.0, plus the Inlet/Outlet grains sensors that are disabled by default. Target/auto/continuous controls are implemented; faults, purge, locator, draining/defrosting status, the specific-humidity display select, reported working time and coil temperature remain registered but unavailable locally. Review dependent automations and actual entity IDs before migration. ON requires the local power option, disabled by default. A standalone local endpoint has confirmed ON/OFF, targets 50%/55% and continuous-mode restoration. A persistent Home Assistant installation also confirmed ON/OFF while retaining continuous mode. Each installation still requires appliance commissioning; an earlier mode-change failure, other numeric targets, cold boot and sustained offline operation remain unresolved or unverified. There is no automatic cloud fallback. See [Reconfiguration and rollback](LOCAL_CONTROL.md#reconfigure-an-existing-cloud-entry).

### Switching connections

Use **Settings → Devices & services → ALORAIR Lite → Reconfigure** on the existing entry. Select the desired connection and complete its setup. Local still requires device-specific routing, and cloud requires restoring the device's cloud route plus account authentication. Changing the HA connection setting does not change your router. Keep the existing entry; deleting and re-adding it is unnecessary.

Starting with 0.3.1, both profiles register the same entity set and use the same feature-based unique IDs. Existing entity IDs, names, enabled/disabled choices and dashboard/automation references are retained. Updating an older cloud installation also adds the Power switch previously offered only locally. Normally disabled diagnostic sensors keep their existing defaults.

A feature unavailable over the selected local connection has state `unavailable` and attribute `unavailable_reason: not_supported_by_local_connection`. That explains a protocol limitation, not a lost connection. A loss of communication also makes supported readings unavailable, without that unsupported-feature reason. Unsupported local control actions are rejected before sending anything. Switching back to a working cloud connection resumes those entities with fresh cloud data; missing or stale data is never substituted with an old value.

This keeps references stable, but automations still need an available measurement to evaluate a condition. Since 0.4.0 local mode does provide the dehumidifier's own inlet/outlet humidity, temperature and specific humidity, decoded from its native status frames; it still provides no fault, purge or locate information, and it never substitutes another room sensor or a retained cloud value. Local measurements become unavailable 35 seconds after the last fresh report. No automatic cloud fallback is enabled.
...
| Local entry has no samples, or a measured value reads unknown | Check [local routing and troubleshooting](LOCAL_CONTROL.md#troubleshooting-and-remaining-work). Since 0.4.0 the native inlet/outlet measurements are decoded, but they go unavailable 35 seconds after the last fresh report and are left unknown rather than guessed when a value fails its range or temperature-conversion check. A reported target is not a humidity measurement, and the local profile does not query cloud status as a fallback. |
