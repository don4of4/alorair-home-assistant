...
For a device already configured with Cloud, use the existing entry's **Reconfigure** action. Duplicate device identities are rejected. Reconfiguration preserves the entry identity and replaces cloud credentials with local settings, but the entity set changes: the local profile has a power switch, display select and diagnostics, with no humidity entity. Review dependent automations; this is not a drop-in migration. ON requires the local power option, disabled by default, and local appliance power commissioning remains pending. There is no automatic cloud fallback. See [Reconfiguration and rollback](LOCAL_CONTROL.md#reconfigure-an-existing-cloud-entry).
...
| Local entry has no samples or lacks humidity entities | Check [local routing and troubleshooting](LOCAL_CONTROL.md#troubleshooting-and-remaining-work). The local profile deliberately has a smaller entity set; it does not query cloud status as a fallback. |
