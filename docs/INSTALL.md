...
This keeps references stable, but automations still need an available measurement to evaluate a condition. Since 0.4.0 local mode provides the dehumidifier's own inlet/outlet measurements, though still no fault, purge or locate information. It never substitutes another room sensor or a retained cloud value, and local measurements become unavailable 35 seconds after the last fresh report. No automatic cloud fallback is enabled.
...
| Local entry has no samples, or a measured value reads unknown | Check [local routing and troubleshooting](LOCAL_CONTROL.md#troubleshooting-and-remaining-work). Measurements go unavailable 35 seconds after the last fresh report, and a value failing its range or conversion check is left unknown. A reported target is not a humidity measurement, and the local profile does not query cloud status as a fallback. |
