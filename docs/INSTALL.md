...
This keeps references stable, but automations still need an available measurement to evaluate a condition. Local mode does not provide the dehumidifier's own humidity readings or silently substitute another room sensor. No automatic cloud fallback is enabled.
