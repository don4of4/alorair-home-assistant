...
| Draining | The existing Draining binary sensor reports active/inactive pump-out | Its native status flag was correlated with a physical purge. This reports state, not water flow, and does not enable the Purge button locally. |
...
Both profiles register the same entities, including optional diagnostics disabled by default. Local mode supports the controls and telemetry listed above. The grains sensors are disabled by default. Fault codes and the fault binary sensor, Defrosting, the Purge and Refresh buttons, Locate, the specific-humidity display select, Reported working time and Coil temperature stay unavailable with `unavailable_reason: not_supported_by_local_connection`. Reconfiguring to cloud resumes supported cloud features through the same entity IDs. No cached cloud readings are substituted, and unsupported actions are rejected. Network routing and Home Assistant reconfiguration are separate steps.
