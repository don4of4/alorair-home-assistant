...
Local ON is disabled by default behind **Allow experimental local power-on**. The native power exchange has been captured, but local live power commissioning is pending, and native faults/humidity control are not mapped. A safety OFF can be attempted on a verified connected session with stale telemetry; a matched later report is still required before claiming confirmation. If the connection is lost, no local command can reach the appliance. Unloading an entry or removing its route does not itself stop an enabled unit.

The verified local evidence is limited to display selection and one warm reconnect on a standalone endpoint. It is not a proof of long-running autonomous drying, cold-boot behavior, or a complete Home Assistant migration. The vendor app may cease updating when the device connection is redirected locally; restoring routing and restoring a cloud entry are separate operations.
