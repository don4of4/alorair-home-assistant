...
| Native measured telemetry mapping | The inlet/outlet humidity, temperature and specific-humidity fields were reverse-engineered from packet captures of this unit's native status reports. They were cross-checked against the vendor cloud's readings of the same sensors. | Not a reference-instrument calibration; live observation on an installed build is pending. |
| Native draining status | A physical purge correlated with the native drain flag changing inactive → active → inactive. | Establishes reported draining state, not water flow or the native remote purge command. |
